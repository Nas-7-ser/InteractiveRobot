...
data = pd.read_csv("karate.dat", sep=' ', header=None)
...
unique_ids = pd.unique(data.values.ravel('K'))
id_to_int = {id_str: i for i, id_str in enumerate(unique_ids)}
int_to_id = {i: id_str for i, id_str in enumerate(unique_ids)}

# Convert string IDs in data to integers
data = data.map(lambda x: id_to_int[x])
